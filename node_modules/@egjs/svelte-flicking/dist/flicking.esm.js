...
version: 4.10.7
*/
import VanillaFlicking, { ExternalRenderer, getFlickingAttached, toArray, VirtualRenderingStrategy, NormalRenderingStrategy, sync, EVENTS, getDefaultCameraTransform, range, withFlickingMethods } from '@egjs/flicking';
import { createEventDispatcher, setContext, onDestroy, onMount, afterUpdate, getContext } from 'svelte';
...
function instance$1($$self, $$props, $$invalidate) {
  ...
  onMount(() => {
    ...
    const rendererOptions = {
      ...
      forceUpdate,
      align: options.align,
      strategy: options.virtual && panelsPerView > 0 ? new VirtualRenderingStrategy() : new NormalRenderingStrategy({
  ...
  afterUpdate(() => {
    ...
    if (panelManager.dirty) {
      ...
      renderEmitter.once("render", () => {
        diffResult = slotDiffer.update(toArray(cameraEl.children)); // As added elements should always back in the slots list

        sync(vanillaFlicking, diffResult, [...diffResult.prevList.map(el => panelManager.get(el.dataset.key)), ...diffResult.added.map(idx => panelManager.get(diffResult.list[idx].dataset.key))]);
...
var Flicking_panel$1 = Flicking_panel;
...
export { Flicking_panel$1 as FlickingPanel, SvelteFlicking as default };
